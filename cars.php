...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-5 animate-up">
        <h2 class="fw-bold m-0 text-primary">Drive Easy - <span><?= htmlspecialchars($typeFilter) ?>s</span></h2>
    </div>
    ...
    <div class="glass p-4 rounded-4 mb-5 animate-up" style="animation-delay: 0.1s;">
        <form class="row g-3" method="GET">
            <div class="col-md-3">
                ...
                <select name="car_type" class="form-select border-0 bg-light">
                ...
                <div class="col">
                    <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden vehicle-card-modern">
                        ...
                        <div class="card-body p-4">
                            <h5 class="fw-bold mb-2"><?= htmlspecialchars($vehicle['vehicle_brand'].' '.$vehicle['vehicle_model']) ?></h5>
                            <div class="d-flex align-items-center mb-4 text-muted small">
                                <span class="me-3"><i class="fas fa-map-marker-alt me-1 text-primary"></i> <?= htmlspecialchars($vehicle['city'] ?? 'Anywhere') ?></span>
                                <span><i class="fas fa-cog me-1 text-primary"></i> Manual/Auto</span>
                            </div>
                            
                            <div class="d-flex align-items-center justify-content-between mt-auto">
                                <?php if ($vehicle['vehicle_availability']==='yes'): ?>
                                    <span class="text-success small fw-bold"><i class="fas fa-circle me-1" style="font-size: 8px;"></i> Available</span>
                                <?php else: ?>
                                    <span class="text-secondary small fw-bold"><i class="fas fa-circle me-1" style="font-size: 8px;"></i> Booked</span>
                                <?php endif; ?>

                                <?php if (is_user_logged_in()): ?>
                                    <a href="my_bookings.php?vehicle_id=<?= $vehicle['vehicle_id'] ?>" class="btn btn-primary btn-sm px-4 rounded-pill">
                                        Book Now
                                    </a>
                                <?php else: ?>
